...
void StatusBar::Paint() {
    ...
    msg_mutex_.unlock();
    snprintf(output_buf.data(), output_buf.size(), " 今日收益: %.2f, 估算总值: %.2f ", income_, sum_);
...
bool StatusBar::MessageProc(const Msg &msg) {
    ...
    if (msg.msg_type == kUpdateIncome) {
        income_ = *reinterpret_cast<float*>(const_cast<void **>(&msg.lparam));
        sum_ = *reinterpret_cast<float*>(const_cast<void**>(&msg.rparam));
        Update();
        processed = true;
